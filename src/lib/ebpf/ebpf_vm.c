...
static bool validate(const struct ebpf_vm* vm, const struct ebpf_inst* insts, uint32_t num_insts, char** errmsg);
static bool bounds_check(const struct ebpf_vm* vm, void* addr, int size, const char* type, uint16_t cur_pc, void* mem, size_t mem_len, void* stack);
...
ebpf_load(struct ebpf_vm* vm, const void* code, uint32_t code_len, char** errmsg)
{
	...
	*errmsg = NULL;
	...
	if (vm->insts) {
		*errmsg = ebpf_error("code has already been loaded into this VM. Use ebpf_unload_code() if you need to reuse this VM");
		return -1;
	}
		...
		switch (inst.opcode) {
			...
			case EBPF_OP_CALL:
				if (inst.imm < 0 || inst.imm >= MAX_EXT_FUNCS) {
					*errmsg = ebpf_error("invalid call immediate %d at PC %d", inst.imm, i);
					return false;
				}
...
static bool
bounds_check(const struct ebpf_vm* vm, void* addr, int size, const char* type, uint16_t cur_pc, void* mem, size_t mem_len, void* stack)
{
	if (!vm->bounds_check_enabled) return true;
	if (mem && (addr >= mem && ((char*)addr + size) <= ((char*)mem + mem_len))) {
		...
		vm->error_printf(stderr, "ebpf error: out of bounds memory %s at PC %u, addr %p, size %d\nmem %p/%zd stack %p/%d\n", type, cur_pc, addr, size,
