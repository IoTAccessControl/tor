...
struct hashmap {
    ...
    uint64_t seed0;
    uint64_t seed1;
    uint64_t (*hash)(const void *item, size_t keysz, uint64_t seed0, uint64_t seed1);
    int (*compare)(const void *a, const void *b, size_t keysz, void *udata);
...
static uint64_t get_hash(struct hashmap *map, const void *key) {
    return clip_hash(map->hash(key, map->keysz, map->seed0, map->seed1));
}
...
struct hashmap *hashmap_new_with_allocator(void *(*_malloc)(size_t), 
    ...
    size_t keysz, size_t elsize, size_t cap, uint64_t seed0, uint64_t seed1,
    uint64_t (*hash)(const void *item, size_t keysz, uint64_t seed0, uint64_t seed1),
    int (*compare)(const void *a, const void *b, size_t keysz, void *udata),
    void (*elfree)(void *item),
    void *udata)
...
struct hashmap *hashmap_new(size_t keysz, size_t elsize, size_t cap, uint64_t seed0, 
    uint64_t seed1,
    uint64_t (*hash)(const void *item, size_t keysz, uint64_t seed0, uint64_t seed1),
    int (*compare)(const void *a, const void *b, size_t keysz, void *udata),
    void (*elfree)(void *item),
    void *udata)
{
    return hashmap_new_with_allocator(NULL, NULL, NULL, keysz, elsize, cap, seed0, 
...
static bool resize0(struct hashmap *map, size_t new_cap) {
    struct hashmap *map2 = hashmap_new_with_allocator(map->malloc, map->realloc, 
        map->free, map->keysz, map->elsize, new_cap, map->seed0, map->seed1, map->hash, 
    ...
    uint64_t hash)
{
    ...
    while(1) {
        ...
        if (entry->hash == bucket->hash && (!map->compare ||
            map->compare(eitem, bitem, map->keysz, map->udata) == 0))
        {
...
const void *hashmap_get(struct hashmap *map, const void *key) {
    // printf("hashmap_get: %d %lu\n", *(uint8_t *)key, get_hash(map, key));
    return hashmap_get_with_hash(map, key, get_hash(map, key));
}

const void *hashmap_set_by_kv(struct hashmap *map, const void *key, const void *val) {
	uint8_t item[128];
    ...
    uint64_t hash)
{
    ...
    while(1) {
        ...
        if (bucket->hash == hash && (!map->compare ||
            map->compare(key, bitem, map->keysz, map->udata) == 0))
        {
            memcpy(map->spare, bitem, map->elsize);
            bucket->dib = 0;
            while(1) {
...
int hashmap_default_compare(const void *a, const void *b, size_t keysz, void *udata) {
    return memcmp(a, b, keysz);
...
static int compare_ints_udata(const void *a, const void *b, size_t keysz, void *udata) {
    return *(int*)a - *(int*)b;
}

static int compare_strs(const void *a, const void *b, size_t keysz, void *udata) {
...
static uint64_t hash_str(const void *item, size_t keysz, uint64_t seed0, uint64_t seed1) {
...
static void all(void) {
    ...
    while (!(map = hashmap_new(0, sizeof(int), 0, seed, seed, 
    ...
    while (!(map = hashmap_new(0, sizeof(char*), 0, seed, seed,
...
static void benchmarks(void) {
    ...
    map = hashmap_new(0, sizeof(int), 0, seed, seed, hash_int, compare_ints_udata, 
                      NULL, NULL);
    bench("set", N, {
    ...
    map = hashmap_new(0, sizeof(int), N, seed, seed, hash_int, compare_ints_udata, 
                      NULL, NULL);
    bench("set (cap)", N, {
        const int *v = hashmap_set(map, &vals[i]);
        assert(!v);
