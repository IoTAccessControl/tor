...
/* eWFD delay handlers
*/
// orginal implementation with no delay support
static int channel_flush_from_first_active_circuit_impl_raw(channel_t *chan, int max);
// use active/inactive to impl delay
static int channel_flush_from_first_active_circuit_impl_basic(channel_t *chan, int max);
...
channel_flush_from_first_active_circuit, (channel_t *chan, int max))
{
  ...
  // return channel_flush_from_first_active_circuit_impl_basic(chan, max);
  return channel_flush_from_first_active_circuit_impl_advance(chan, max);
}
                             ...
                             streamid_t fromstream)
{
  ...
  EWFD_TEMP_LOG("send-cell append to queue circ: %u cell-cmd: %s stream: %d q-len: %d", ewfd_get_circuit_id(circ),
    cell_command_to_string(cell->command), fromstream, queue->n + 1);
  ...
  if (PREDICT_UNLIKELY(cell_queues_check_size())) {
    EWFD_LOG("ERROR: append_cell trigger OOM and close circ: %u", ewfd_get_circuit_id(circ));
